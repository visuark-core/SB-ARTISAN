...
export interface PrimaryButtonProps<T extends React.ElementType = "button"> {
  /**
...
type PolymorphicPrimaryButtonProps<T extends React.ElementType> = PrimaryButtonProps<T> &
  Omit<React.ComponentPropsWithRef<T>, keyof PrimaryButtonProps<T>>;

/**
 * Premium polymorphic solid button for high-end call-to-actions.
 * Features a dark, premium color palette, micro-translations, and custom focus states.
 */
  ...
  <T extends React.ElementType = "button">(
    {
      ...
      children,
      ...props
    }: PolymorphicPrimaryButtonProps<T>,
    ref: React.Ref<any>
  ) => {
    ...
    return (
      <Component
        ...
        className={cn(
          ...
          // Color system: Dark luxury solid background to soft charcoal hover
          "bg-[#1A1A1A] hover:bg-[#333230] text-[#FDFCF7] border border-[#1A1A1A] hover:border-[#333230]",
          ...
          isDisabled && "opacity-50 cursor-not-allowed hover:bg-[#1A1A1A] hover:border-[#1A1A1A]",
...
(PrimaryButton as any).displayName = "PrimaryButton";

export default PrimaryButton;
export type { PolymorphicPrimaryButtonProps };
